...
import {
  ...
  ReelSpinSettled,
  useGame,
  WinrOfOlympus1000FormFields,
  WinrOfOlympus1000Template,
} from '@winrlabs/games';
import {
  controllerAbi,
  useCurrentAccount,
...
const log = debug('worker:WinrOfOlympus1000Web3');
...
export default function WinrOfOlympus1000Game({
  buildedGameUrl,
  ...
  const [formValues, setFormValues] = React.useState<WinrOfOlympus1000FormFields>({
    betAmount: 1,
    actualBetAmount: 1,
    isDoubleChance: false,
  });

  const gameEvent = useListenGameEvent(gameAddresses.winrOfOlympus1000);
  ...
  const gameDataRead = useReadContract({
    ...
    address: gameAddresses.winrOfOlympus1000 as `0x${string}`,
