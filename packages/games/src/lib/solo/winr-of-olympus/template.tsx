...
import { useUnityOlympus } from "./hooks/use-olympus-unity";
import {
  WinrOfOlympusFormFields,
  WinrOfOlympusReelSpinSettled,
  WinrOfOlympusSpinType,
  WinrOfOlympus_Unity_Events,
} from "./types";
import React from "react";
import { toDecimals, toFormatted } from "../../utils/web3";
import { wait } from "../../utils/promise";
import { useOlympusGameStore } from "./store";
...
interface TemplateProps {
  ...
  gameEvent: WinrOfOlympusReelSpinSettled;
...
export const WinrOfOlympusTemplate = ({
  ...
  const {
    ...
    hideFreeSpinText,
    handleSpinStatus,
  } = useUnityOlympus({ buildedGameUrl, buildedGameUrlMobile });
  ...
  const handleMessageFromUnity = React.useCallback(
    (name: string, strParam: string) => {
      ...
      setTimeout(() => {
        if (obj.name === WinrOfOlympus_Unity_Events.BET) {
        ...
        if (obj.name === WinrOfOlympus_Unity_Events.CHANGE_BET) {
          setBetAmount(Number(obj.strParam));
        }

        if (obj.name === WinrOfOlympus_Unity_Events.DOUBLE_CHANCE_CLICK) {
        ...
        if (obj.name === WinrOfOlympus_Unity_Events.GRID_ANIMATION_FINISHED) {
        ...
        if (obj.name === WinrOfOlympus_Unity_Events.BUY_FEATURE_CLICK) {
          handleBuy();
        ...
        if (
          obj.name === WinrOfOlympus_Unity_Events.CLOSED_CONGRATULATIONS_PANEL
        ) {
          if (
            isInFreeSpinMode &&
            initialBuyEvent &&
  ...
  React.useEffect(() => {
    if (currentAction == "submit" && gameEvent?.type == "Game") {
      ...
      if (gameEvent.payoutMultiplier > 0) {
        ...
        if (gameEvent.spinType === WinrOfOlympusSpinType.DOUBLE_CHANCE) {
